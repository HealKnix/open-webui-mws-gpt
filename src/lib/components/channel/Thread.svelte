...
{#if channel}
  <div class="dark:bg-gray-850 flex h-full w-full flex-col bg-gray-50">
    <div class="sticky top-0 flex items-center justify-between px-3.5 py-3">
      <div class=" text-lg font-medium">{$i18n.t('Thread')}</div>
    ...
    <div class=" max-h-full w-full overflow-y-auto" bind:this={messagesContainerElement}>
      {#if messages !== null}
        ...
        />
      {:else}
        <div class="flex w-full justify-center pt-5 pb-10">
          <Spinner />
        </div>
      {/if}

      <div class=" w-full px-2.5 pb-[1rem]">
        <MessageInput
          bind:replyToMessage
