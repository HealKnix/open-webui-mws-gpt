  ...
  <div>
    ...
    {:else if !thread}
      <div class="mx-auto max-w-full px-5">
        {#if channel}
          <div class="flex flex-col gap-1.5 pt-10 pb-5">
            {#if channel?.type === 'dm'}
              <div class="mr-0.5 ml-[1px] flex">
        ...
        {:else}
          <div class="flex items-center justify-center gap-2 py-5 text-xs">
            <div class=" ">{$i18n.t('Start of the channel')}</div>
          </div>
        {/if}

        {#if messageList.length > 0}
          <hr class=" w-full border-gray-50 py-2.5 dark:border-gray-700/20" />
        {/if}
      </div>
    {/if}
