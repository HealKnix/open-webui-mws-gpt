...
<div class="-mx-1 text-xs">
  ...
  <button
    type="button"
    class="flex w-full items-center gap-3 rounded-xl px-3.5 py-3 transition hover:bg-gray-50 dark:hover:bg-gray-900"
    on:click={onClick}
  >
    <img
  ...
  <!-- Expanded -->
  {#if expanded}
    <div class="dark:border-gray-850 mt-1 mb-3 rounded-2xl border border-gray-100 px-3.5 py-3">
      <div class="flex items-center gap-3">
        <button
          type="button"
          class="shrink-0 overflow-hidden rounded-xl transition hover:opacity-80"
        ...
        <div class="flex-1">
          <div class=" text-xs text-gray-500">{$i18n.t('Name')}</div>
          <input
            type="text"
            class="w-full bg-transparent text-sm outline-none placeholder:text-gray-300 dark:placeholder:text-gray-700"
            bind:value={name}
