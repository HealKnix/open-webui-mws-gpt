...
<div
  ...
  class=" w-full {className} group relative flex rounded-xl hover:bg-gray-100 dark:hover:bg-gray-900 {$page
    ...
    : ' text-gray-600 dark:text-gray-400'} cursor-pointer select-none"
>
  <a
  ...
  >
    <div class="flex items-center gap-1">
      <div>
        {#if channel?.type === 'dm'}
          {#if channel?.users}
            ...
            <div class="relative mr-0.5 ml-[1px] flex">
          ...
          {/if}
        {:else}
          <div class=" ml-1 flex size-4 items-center justify-center">
      ...
      <div
        class=" line-clamp-1 flex w-full flex-1 items-center gap-2.5 self-center overflow-hidden pr-1 text-left"
      >
        {#if channel?.name}
        ...
        {:else}
          ...
          {#if channel?.users?.length === 2}
            ...
            {#if dmUser?.status_emoji || dmUser?.status_message}
              <span class="line-clamp-1 flex gap-1.5">
    ...
    <div class="flex items-center">
      {#if channel?.unread_count > 0}
        <div
          class="rounded-xl bg-gray-100 px-2 py-[1px] text-xs font-medium whitespace-nowrap text-black dark:bg-gray-800 dark:text-white"
        >
  ...
  {#if ['dm'].includes(channel?.type)}
    <div
      class="invisible mr-1 ml-0.5 flex items-center self-center group-hover:visible dark:text-gray-300"
    >
      <button
        type="button"
