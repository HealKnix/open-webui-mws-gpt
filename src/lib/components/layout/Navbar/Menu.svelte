...
{#if showFullMessages}
  <div class="hidden h-full w-full flex-col">
...
>
  ...
  <div slot="content">
    <div
      class="dark:bg-gray-850 z-50 max-w-[200px] min-w-[200px] rounded-2xl border border-gray-100 bg-white px-1 py-1 shadow-lg transition select-none dark:border-gray-800 dark:text-white"
    >
      ...
      <DropdownSub>
        ...
        {#if $user?.role === 'admin' || ($user.permissions?.chat?.export ?? true)}
          ...
          >
            <div class="line-clamp-1 flex items-center">{$i18n.t('Export chat (.json)')}</div>
          </button>
        {/if}
        <button
          draggable="false"
          ...
          <DropdownSub maxWidth={200}>
            ...
            {#each $folders.sort((a, b) => b.updated_at - a.updated_at) as folder}
              {#if folder?.id}
                <button
                  draggable="false"
                  class="flex w-full cursor-pointer items-center gap-2 overflow-hidden rounded-xl px-3 py-1.5 text-sm hover:bg-gray-50 dark:hover:bg-gray-800"
                  on:click={() => {
