...
{#if models !== null}
  {#if selectedModelId === null}
    <div class="mt-1.5 mb-2 flex flex-col gap-1">
    ...
    >
      ...
      <div class="my-2 px-3" id="model-list">
        {#if filteredModels.length > 0}
          {#each filteredModels.slice((currentPage - 1) * perPage, currentPage * perPage) as model, modelIdx (`${model.id}-${modelIdx}`)}
            <div
              class=" flex w-full cursor-pointer space-x-4 rounded-xl px-3 py-2 transition hover:bg-black/5 dark:hover:bg-white/5 {model
                ?.meta?.hidden
            ...
            >
              <button
                class=" flex w-full flex-1 cursor-pointer space-x-3.5 text-left"
                type="button"
                on:click={() => {
                  selectedModelId = model.id;
                }}
              >
                <div class=" w-9 self-center">
                  <div
                ...
                >
                  ...
                  <div
                    class=" line-clamp-1 flex items-center gap-1 overflow-hidden text-xs text-ellipsis text-gray-500"
                  >
              ...
              <div class="flex flex-row items-center gap-0.5 self-center">
                {#if shiftKey}
