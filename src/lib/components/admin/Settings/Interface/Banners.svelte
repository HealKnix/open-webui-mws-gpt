...
<div class=" flex flex-col gap-3 {banners?.length > 0 ? 'mt-2' : ''}" bind:this={bannerListElement}>
  {#each banners as banner, bannerIdx (banner.id)}
    <div class=" -ml-1 flex items-start justify-between" id="banner-item-{banner.id}">
      <EllipsisVertical className="size-4 cursor-move item-handle" />

      <div class="flex flex-1 flex-row items-start gap-2">
        <select
          class="w-fit rounded-xl bg-transparent pr-5 pl-1 text-xs capitalize outline-hidden"
