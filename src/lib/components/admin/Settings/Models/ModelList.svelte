...
{#if modelIds.length > 0}
  <div class="flex -translate-x-1 flex-col" bind:this={modelListElement}>
    {#each modelIds as modelId (modelId)}
      <div class=" flex w-full items-center justify-between gap-2" id="model-item-{modelId}">
        <Tooltip content={modelId} placement="top-start">
          <div class="flex items-center gap-1">
            <EllipsisVertical className="size-4 cursor-move model-item-handle" />

            <div class=" line-clamp-1 flex-1 rounded-lg py-1 text-sm">
