  ...
  {#if RAGConfig}
    <div class="scrollbar-hidden h-full space-y-2.5 overflow-y-scroll p-1">
      <div class="space-y-1">
        ...
        <SettingItem label={$i18n.t('Content Extraction Engine')}>
          <select
        ...
        {#if RAGConfig.CONTENT_EXTRACTION_ENGINE === ''}
          ...
          <SettingItem label={$i18n.t('PDF Loader Mode')}>
            <div slot="label">
              <Tooltip
                content={$i18n.t(
        ...
        {:else if RAGConfig.CONTENT_EXTRACTION_ENGINE === 'datalab_marker'}
          ...
          <ToggleSetting
            ...
            bind:state={RAGConfig.DATALAB_MARKER_USE_LLM}
            tooltip={$i18n.t(
          ...
          <ToggleSetting
            ...
            bind:state={RAGConfig.DATALAB_MARKER_SKIP_CACHE}
            tooltip={$i18n.t('Skip the cache and re-run the inference. Defaults to False.')}
          />
          <ToggleSetting
            label={$i18n.t('Force OCR')}
            bind:state={RAGConfig.DATALAB_MARKER_FORCE_OCR}
            tooltip={$i18n.t(
          ...
          <ToggleSetting
            ...
            bind:state={RAGConfig.DATALAB_MARKER_PAGINATE}
            tooltip={$i18n.t(
          ...
          <ToggleSetting
            ...
            bind:state={RAGConfig.DATALAB_MARKER_FORMAT_LINES}
            tooltip={$i18n.t(
          ...
          />
          <SettingItem label={$i18n.t('Output Format')}>
            <div slot="label">
              <Tooltip
                content={$i18n.t(
        ...
        {:else if RAGConfig.CONTENT_EXTRACTION_ENGINE === 'mineru'}
          <SettingItem label={$i18n.t('API Mode')}>
            <select
              class="w-fit rounded-sm bg-transparent px-2 pr-8 text-right text-xs outline-hidden"
          ...
          <SettingItem label={$i18n.t('API Timeout')}>
            <input
              class="w-16 bg-transparent text-right text-sm font-medium outline-hidden"
              type="number"
              min="1"
        ...
        <ToggleSetting
          ...
          tooltip={RAGConfig.BYPASS_EMBEDDING_AND_RETRIEVAL
            ? $i18n.t(
        ...
        {#if !RAGConfig.BYPASS_EMBEDDING_AND_RETRIEVAL}
          <SettingItem label={$i18n.t('Text Splitter')}>
            <select
              class="w-fit rounded-sm bg-transparent px-2 pr-8 text-right text-xs outline-hidden"
            ...
            <div class=" flex w-full gap-1.5">
              <div class="  w-full justify-between">
                ...
                <div class="self-center">
                  <input
                    class=" dark:bg-gray-850 w-full rounded-lg bg-gray-50 px-4 py-1.5 text-sm outline-hidden dark:text-gray-300"
                    type="number"
      ...
      {#if !RAGConfig.BYPASS_EMBEDDING_AND_RETRIEVAL}
        <div class="space-y-1">
          ...
          <div class="  mb-2.5 flex w-full flex-col justify-between">
            <SettingItem label={$i18n.t('Embedding Model Engine')}>
              <select
            ...
            {:else if RAG_EMBEDDING_ENGINE === 'azure_openai'}
              <div class="my-0.5 flex w-full flex-col gap-2 pr-2">
              ...
              {:else}
                <div class="flex w-full">
                  ...
                  {#if RAG_EMBEDDING_ENGINE === ''}
                    <button
                      class="rounded-lg bg-transparent px-2.5 text-gray-800 transition dark:bg-transparent dark:text-gray-100"
                      on:click={() => {
          ...
          <SettingItem label={$i18n.t('Embedding Batch Size')}>
            <input
              bind:value={RAG_EMBEDDING_BATCH_SIZE}
              type="number"
              class="w-14 bg-transparent text-center outline-none"
              min="-2"
              max="16000"
              step="1"
            />
          ...
          {#if RAG_EMBEDDING_ENGINE === 'ollama' || RAG_EMBEDDING_ENGINE === 'openai' || RAG_EMBEDDING_ENGINE === 'azure_openai'}
            <ToggleSetting
              ...
              bind:state={ENABLE_ASYNC_EMBEDDING}
              tooltip={$i18n.t(
        ...
        <div class="space-y-1">
          ...
          <ToggleSetting
            ...
            tooltip={RAGConfig.RAG_FULL_CONTEXT
              ? $i18n.t(
                  'Inject the entire content as context for comprehensive processing, this is recommended for complex queries.',
                )
              : $i18n.t(
          ...
          {#if !RAGConfig.RAG_FULL_CONTEXT}
            ...
            <SettingItem label={$i18n.t('Top K')}>
              <input
                class="w-14 bg-transparent text-center outline-none"
                type="number"
            ...
            {#if RAGConfig.ENABLE_RAG_HYBRID_SEARCH === true}
              <div class=" mb-2.5 w-full justify-between py-0.5">
                <Tooltip
                  content={$i18n.t(
                ...
                {#if (RAGConfig?.HYBRID_BM25_WEIGHT ?? null) !== null}
                  <div class="mt-0.5 flex space-x-2">
                    <div class=" flex-1">
                      ...
                      <div class="py-0.5">
                        <div class="flex w-full justify-between">
                          <div class=" font-small text-left text-xs">
                            {$i18n.t('semantic')}
                          </div>
                          <div class=" font-small text-right text-xs">
                            {$i18n.t('lexical')}
                          </div>
                        </div>
                      </div>
      ...
      <div class="space-y-1">
        ...
        <SettingItem label={$i18n.t('Allowed File Extensions')}>
          <div class="relative flex items-center">
            <Tooltip
              content={$i18n.t(
        ...
        <SettingItem label={$i18n.t('Max Upload Size')}>
          <div class="relative flex items-center">
            <Tooltip
              content={$i18n.t(
        ...
        <SettingItem label={$i18n.t('Max Upload Count')}>
          <div class="relative flex items-center">
            <Tooltip
              content={$i18n.t(
        ...
        <SettingItem label={$i18n.t('Image Compression Width')}>
          <div class="relative flex items-center">
            <Tooltip
              content={$i18n.t(
        ...
        <SettingItem label={$i18n.t('Image Compression Height')}>
          <div class="relative flex items-center">
            <Tooltip
              content={$i18n.t(
      ...
      <div class="space-y-1">
        ...
        <SettingItem label={$i18n.t('Reset Upload Directory')}>
          <div class="relative flex items-center">
            <button
              class="text-xs"
              type="button"
        ...
        <SettingItem label={$i18n.t('Reset Vector Storage/Knowledge')}>
          <div class="relative flex items-center">
            <button
              class="text-xs"
              type="button"
        ...
        <SettingItem label={$i18n.t('Reindex Knowledge Base Vectors')}>
          <div class="relative flex items-center">
            <button
              class="text-xs"
              type="button"
