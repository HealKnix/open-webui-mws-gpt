...
<Modal size="md" bind:show>
  {#if model}
    ...
    <!-- Tabs -->
    <div class="dark:border-gray-850 border-b border-gray-100 px-5">
      <div class="flex gap-4">
        <button
          class="border-b-2 py-2 text-sm font-medium transition-colors {selectedTab === 'overview'
        ...
        {#if $config?.features?.enable_admin_chat_access}
          <button
            class="border-b-2 py-2 text-sm font-medium transition-colors {selectedTab === 'chats'
              ? 'border-black text-gray-900 dark:border-white dark:text-white'
              : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}"
        ...
        <div class="mt-3 mb-4">
          <div class="mb-2 flex items-center justify-between">
            <Tooltip content={$i18n.t('Thumbs up/down ratings from users on model responses')}>
              <div class="cursor-help text-xs font-medium tracking-wide text-gray-500 uppercase">
