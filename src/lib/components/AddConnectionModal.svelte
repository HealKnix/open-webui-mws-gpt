      ...
      <div class=" flex w-full flex-col sm:flex-row sm:justify-center sm:space-x-6">
        ...
        >
          <div class="px-1">
            ...
            {#if !ollama && !direct}
              <div class="mt-1 flex w-full flex-row items-center justify-between">
                <label
                  for="api-type-toggle"
                  class={`mb-0.5 text-xs text-gray-500
							${($settings?.highContrastMode ?? false) ? 'text-gray-800 dark:text-gray-100' : ''}`}
            ...
            <div class="mt-2 flex w-full flex-col">
              ...
              {:else}
                <div
                  class={`px-10 py-2 text-center text-xs text-gray-500
								${($settings?.highContrastMode ?? false) ? 'text-gray-800 dark:text-gray-100' : ''}`}
                >
                  {#if ollama}
            ...
            <div class="flex items-center">
              ...
              <input
                class="w-full rounded-lg bg-transparent py-1 text-sm {modelId
                  ? ''
                  : 'text-gray-500'} {($settings?.highContrastMode ?? false)
                  ? 'placeholder:text-gray-700 dark:placeholder:text-gray-100'
                  : 'outline-hidden placeholder:text-gray-300 dark:placeholder:text-gray-700'}"
                bind:value={modelId}
