          ...
          <div class="px-1">
              ...
              {#if !direct}
                <div class="flex flex-1 flex-col">
                  ...
                  <div class="flex flex-1 items-center">
                    <input
                      id="enter-id"
                      class={`w-full flex-1 bg-transparent font-mono text-sm ${($settings?.highContrastMode ?? false) ? 'placeholder:text-gray-700 dark:placeholder:text-gray-100' : 'outline-hidden placeholder:text-gray-300 dark:placeholder:text-gray-700'}`}
                      type="text"
                      bind:value={id}
                      placeholder="auto"
                      autocomplete="off"
            ...
            <div class="mt-1 mb-1.5 flex w-full flex-col">
              ...
              <div class="flex-1">
                <input
                  id="description"
                  class={`w-full bg-transparent text-sm ${($settings?.highContrastMode ?? false) ? 'placeholder:text-gray-700 dark:placeholder:text-gray-100' : 'outline-hidden placeholder:text-gray-300 dark:placeholder:text-gray-700'}`}
                  type="text"
                  ...
                  {#if ['oauth_2.1', 'oauth_2.1_static'].includes(auth_type)}
                    <div class="flex items-center gap-2">
                      <div class="flex shrink-0 flex-col items-center justify-end">
                        ...
                        >
                          <button
                            class=" text-xs text-gray-700 underline transition hover:text-gray-900 dark:text-gray-500 dark:hover:text-gray-200"
                            type="button"
                            on:click={() => {
                      ...
                      {#if !oauthClientInfo}
                        <div
                          class="rounded-md bg-yellow-500/20 px-1.5 text-xs font-medium text-yellow-700 dark:text-yellow-200"
                        >
                          {$i18n.t('Not Registered')}
                        </div>
                      {:else}
                        <div
                          class="rounded-md bg-green-500/20 px-1.5 text-xs font-medium text-green-700 dark:text-green-200"
                        >
                          {$i18n.t('Registered')}
                        </div>
                      {/if}
                ...
                <div class="flex gap-2">
                  <div class="flex-shrink-0 self-start">
                    <select
                      ...
                      class={`w-full bg-transparent pr-5 text-sm dark:bg-gray-900 ${($settings?.highContrastMode ?? false) ? 'placeholder:text-gray-700 dark:placeholder:text-gray-100' : 'outline-hidden placeholder:text-gray-300 dark:placeholder:text-gray-700'}`}
                  ...
                  <div class="flex flex-1 items-center">
                    ...
                    {:else if auth_type === 'none'}
                      <div
                        class={`translate-y-[1px] self-center text-xs ${($settings?.highContrastMode ?? false) ? 'text-gray-800 dark:text-gray-100' : 'text-gray-500'}`}
                      >
                    ...
                    {:else if auth_type === 'oauth_2.1'}
                      <div
                        class={`flex translate-y-[1px] items-center self-center text-xs ${($settings?.highContrastMode ?? false) ? 'text-gray-800 dark:text-gray-100' : 'text-gray-500'}`}
                      >
                        {$i18n.t('Uses OAuth 2.1 Dynamic Client Registration')}
                      </div>
                    {:else if auth_type === 'oauth_2.1_static'}
                      <div class="mt-0.5 flex w-full flex-col gap-1.5">
                        <SensitiveInput
            ...
            <div class="flex items-center justify-between">
              ...
              {#if !direct}
                <button
                  class="dark:bg-gray-850 mt-2 flex items-center gap-1 rounded-full bg-gray-50 object-cover px-2 py-1 text-black transition hover:bg-gray-100 dark:text-white dark:hover:bg-gray-800"
                  type="button"
                  on:click={() => {
                ...
                <div class="mt-2 flex gap-2">
                  <div class="flex w-full flex-col">
                    ...
                    <div class="flex gap-2">
                      ...
                      <div class="flex flex-1 items-center">
                        ...
                        {:else if spec_type === 'json'}
                          <div
                            class={`w-full translate-y-[1px] self-center text-xs ${($settings?.highContrastMode ?? false) ? 'text-gray-800 dark:text-gray-100' : 'text-gray-500'}`}
                          >
                            <label for="url-or-path" class="sr-only">{$i18n.t('JSON Spec')}</label>
                            <textarea
                              class={`w-full bg-transparent text-sm ${($settings?.highContrastMode ?? false) ? 'placeholder:text-gray-700 dark:placeholder:text-gray-100' : 'text-black outline-hidden placeholder:text-gray-300 dark:text-white dark:placeholder:text-gray-700'}`}
                              bind:value={spec}
                    ...
                    {#if ['', 'url'].includes(spec_type)}
                      <div
                        class={`mt-1 text-xs ${($settings?.highContrastMode ?? false) ? 'text-gray-800 dark:text-gray-100' : 'text-gray-500'}`}
                      >
            ...
            {#if !direct}
              ...
              <div class="mt-2 flex w-full flex-col">
                <label
                  ...
                  class={`mb-1 text-xs ${($settings?.highContrastMode ?? false) ? 'text-gray-800 placeholder:text-gray-700 dark:text-gray-100 dark:placeholder:text-gray-100' : 'text-gray-500 outline-hidden placeholder:text-gray-300 dark:placeholder:text-gray-700'}`}
