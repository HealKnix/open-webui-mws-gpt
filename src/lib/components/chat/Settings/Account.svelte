...
<div id="tab-account" class="flex h-full flex-col justify-between text-sm">
  <div class=" max-h-[28rem] overflow-y-scroll md:max-h-full">
    ...
    <hr class="dark:border-gray-850/30 my-4 border-gray-50" />
    ...
    {#if ($config?.features?.enable_api_keys ?? true) && ($user?.role === 'admin' || ($user?.permissions?.features?.api_keys ?? false))}
      ...
      {#if showAPIKeys}
        <div class="flex flex-col">
          {#if $user?.role === 'admin'}
            <div class="mt-2 w-full justify-between">
              <div class="flex w-full justify-between">
                <div class="mb-1 self-center text-xs font-medium">{$i18n.t('JWT Token')}</div>
              </div>
              ...
              <div class="flex">
                ...
                <button
                  class="dark:hover:bg-gray-850 ml-1.5 rounded-lg px-1.5 py-1 transition"
          ...
          {#if ($config?.features?.enable_api_keys ?? true) && ($user?.role === 'admin' || ($user?.permissions?.features?.api_keys ?? false))}
            <div class="mt-2 w-full justify-between">
              {#if $user?.role === 'admin'}
                <div class="flex w-full justify-between">
                  <div class="mb-1 self-center text-xs font-medium">{$i18n.t('API Key')}</div>
                </div>
              {/if}
              <div class="flex">
                {#if APIKey}
                  ...
                  <Tooltip content={$i18n.t('Create new key')}>
                    <button
                      class=" dark:hover:bg-gray-850transition rounded-lg px-1.5 py-1"
                ...
                {:else}
                  <button
                    class="dark:bg-gray-850 dark:hover:bg-gray-850 flex items-center gap-1.5 rounded-lg bg-gray-100/70 px-3.5 py-1.5 font-medium transition hover:bg-gray-100"
                    on:click={() => {
                      createAPIKeyHandler();
                    }}
                  >
