...
<div class="flex w-full justify-between">
  ...
  <div class="flex h-full flex-shrink-0 justify-end space-x-1 self-start text-xs">
    {#each shortcut.keys.filter((key) => !(key.toLowerCase() === 'delete' && shortcut.keys.includes('Backspace'))) as key}
      <div
        class="flex h-fit items-start justify-center rounded-sm border border-black/10 px-1 py-0.5 text-gray-600 capitalize dark:border-white/10 dark:text-gray-300"
      >
        {formatKey(key)}
      </div>
    {/each}
