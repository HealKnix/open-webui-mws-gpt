...
{#if folder}
  ...
  <div class="group relative mb-3 flex w-full items-center justify-between px-6 @md:max-w-3xl">
    <div class="flex items-center gap-3.5 text-center">
      <EmojiPicker
        onClose={() => {}}
      ...
      >
        <button
          ...
          class=" flex size-11 items-center justify-center rounded-full bg-gray-50 dark:bg-gray-800"
        >
          {#if folder?.meta?.icon}
      ...
      <div class="line-clamp-1 text-3xl">
        {folder.name}
      </div>
    </div>

    <div class="flex translate-x-2.5 items-center">
      <FolderMenu
        align="end"
        onEdit={() => {
      ...
      >
        <button
          class="dark:hover:bg-gray-850 touch-auto rounded-full p-1.5"
