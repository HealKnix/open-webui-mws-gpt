...
<Modal bind:show>
  <div class="px-5 py-4 text-gray-700 dark:text-gray-100">
    <div class="flex justify-between pb-2 dark:text-gray-300">
      <div class="self-center text-lg font-medium">{$i18n.t('Keyboard Shortcuts')}</div>
    ...
    {#each Object.entries(categorizedShortcuts) as [category, items], categoryIndex}
      {#if categoryIndex > 0}
        <div class="py-3">
          <div class="dark:border-gray-850 w-full border-t border-gray-50" />
        </div>
      {/if}

      <div class="flex justify-between pb-2 dark:text-gray-300">
        <div class="self-center text-base">{$i18n.t(category)}</div>
      </div>
      <div class="flex w-full flex-col md:flex-row md:space-x-2 dark:text-gray-200">
        <div class="flex w-full flex-col sm:flex-row sm:justify-center sm:space-x-6">
          <div class=" grid w-full grid-cols-1 gap-2 gap-x-4 sm:grid-cols-2">
            <!-- {$i18n.t('Chat')} -->
            <!-- {$i18n.t('Global')} -->
            <!-- {$i18n.t('Input')} -->
            <!-- {$i18n.t('Message')} -->
