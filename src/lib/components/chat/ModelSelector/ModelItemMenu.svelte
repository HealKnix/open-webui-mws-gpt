...
>
  ...
  <div slot="content">
    <div
      class="dark:bg-gray-850 z-[9999999] min-w-[210px] rounded-2xl border border-gray-100 bg-white p-1 text-sm shadow-lg dark:border-gray-800 dark:text-white"
    >
      {#if model?.preset || model?.info?.base_model_id ? model?.info?.user_id === $user?.id : $user?.role === 'admin'}
        <button
          type="button"
          class="flex w-full items-center gap-2 rounded-xl px-3 py-1.5 transition select-none hover:bg-gray-50 dark:hover:bg-gray-800"
          on:click={(e) => {
            e.stopPropagation();
            e.preventDefault();
        ...
        <hr class="my-1 border-gray-50 dark:border-gray-800/30" />
      {/if}
