...
>
      ...
      <div class="flex min-w-fit items-center">
        <Tooltip content={$user?.role === 'admin' ? (item?.value ?? '') : ''} placement="top-start">
          <img
            ...
            class="flex size-5 items-center rounded-full"
            loading="lazy"
            on:error={(e) => {
      ...
      <div class=" flex shrink-0 items-center gap-2">
        {#if item.model.owned_by === 'ollama'}
          {#if (item.model.ollama?.details?.parameter_size ?? '') !== ''}
            <div class="flex translate-y-[0.5px] items-center">
              <Tooltip
                content={`${
                ...
                className="self-end"
              >
                <span class=" line-clamp-1 text-xs font-medium text-gray-600 dark:text-gray-400"
          ...
          {#if item.model.ollama?.expires_at && new Date(item.model.ollama?.expires_at * 1000) > new Date()}
            <div class="flex translate-y-[0.5px] items-center px-0.5">
              <Tooltip
          ...
          {#key item.model.id}
            <Tooltip elementId="tags-{item.model.id}">
              <div slot="tooltip" id="tags-{item.model.id}">
                {#each item.model?.tags.sort((a, b) => a.name.localeCompare(b.name)) as tag}
                  <Tooltip content={tag.name} className="flex-shrink-0">
                    <div class=" rounded-sm text-xs font-medium text-white uppercase">
                      {tag.name}
                    </div>
                  </Tooltip>
                {/each}
  ...
  <div class="ml-auto flex shrink-0 items-center gap-1.5 pr-1 pl-2">
