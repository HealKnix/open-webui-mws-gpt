          ...
          <div {...wrapperProps}>
            <div
              ...
              class="{props.class} z-40 {$mobile
                ? `w-full`
                : `${className}`} dark:bg-gray-850 max-w-[calc(100vw-1rem)] justify-start rounded-2xl bg-white shadow-lg outline-hidden dark:text-white"
              transition:flyAndScale
            >
              <slot>
                {#if searchEnabled}
                  <div class="mb-1.5 flex items-center gap-2.5 px-4.5 pt-3.5">
                ...
                <div class="px-2">
                  {#if tags && items.filter((item) => !(item.model?.info?.meta?.hidden ?? false)).length > 0}
                    <div
                      class=" dark:bg-gray-850 scrollbar-none mb-0.5 flex w-full overflow-x-auto bg-white font-[450]"
                      on:wheel={(e) => {
                        if (e.deltaY !== 0) {
                          e.preventDefault();
                    ...
                    >
                      <div
                        class="flex w-fit gap-1 rounded-full bg-transparent px-1.5 text-center text-sm whitespace-nowrap"
                        bind:this={tagsContainerElement}
                      >
                        {#if items.find((item) => item.model?.connection_type === 'local') || items.find((item) => item.model?.connection_type === 'external') || items.find((item) => item.model?.direct) || tags.length > 0}
                          <button
                            class="min-w-fit px-1.5 py-0.5 outline-none {selectedTag === '' &&
                            selectedConnectionType === ''
                              ? ''
                              : 'text-gray-300 hover:text-gray-700 dark:text-gray-600 dark:hover:text-white'} capitalize transition"
                        ...
                        {#if items.find((item) => item.model?.connection_type === 'local')}
                          <button
                            class="min-w-fit px-1.5 py-0.5 outline-none {selectedConnectionType ===
                            'local'
                              ? ''
                              : 'text-gray-300 hover:text-gray-700 dark:text-gray-600 dark:hover:text-white'} capitalize transition"
                        ...
                        {#each tags as tag}
                          <Tooltip content={tag}>
                            <button
                              class="min-w-fit px-1.5 py-0.5 outline-none {selectedTag === tag
                                ? ''
                                : 'text-gray-300 hover:text-gray-700 dark:text-gray-600 dark:hover:text-white'} capitalize transition"
                ...
                <div class="group relative px-2.5">
                    ...
                    {#if items.length === 0 && $user?.role === 'admin'}
                      <div class="flex flex-col items-start justify-center px-4 py-6 text-start">
                        ...
                        <a
                          ...
                          class="rounded-xl bg-gray-900 px-4 py-1.5 text-xs font-medium text-white transition hover:bg-gray-800 dark:bg-white dark:text-gray-900 dark:hover:bg-gray-100"
                          on:click={() => {
                            show = false;
                          }}
                        >
                  ...
                  {#if !(searchValue.trim() in $MODEL_DOWNLOAD_POOL) && searchValue && ollamaVersion && $user?.role === 'admin'}
                    ...
                    >
                      <button
                        class="rounded-button data-highlighted:bg-muted line-clamp-1 flex w-full cursor-pointer items-center rounded-xl py-2 pr-1.5 pl-3 text-sm font-medium text-gray-700 outline-hidden transition-all duration-75 select-none hover:bg-gray-100 dark:text-gray-100 dark:hover:bg-gray-800"
                        on:click={() => {
                          pullModelHandler();
                        }}
                      >
                  ...
                  {#each Object.keys($MODEL_DOWNLOAD_POOL) as model}
                    <div
                      class="rounded-button data-highlighted:bg-muted flex w-full cursor-pointer justify-between rounded-xl py-2 pr-1.5 pl-3 text-sm font-medium text-gray-700 outline-hidden transition-all duration-75 select-none dark:text-gray-100"
                    >
                      <div class="flex">
                        ...
                        <div class="flex flex-col self-start">
                          ...
                          {#if 'digest' in $MODEL_DOWNLOAD_POOL[model] && $MODEL_DOWNLOAD_POOL[model].digest}
                            <div class="-mt-1 line-clamp-1 h-fit text-[0.7rem] dark:text-gray-500">
