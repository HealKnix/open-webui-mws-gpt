...
{#if filteredItems.length > 0 || query.startsWith('http')}
  {#each filteredItems as item, idx}
    ...
    {#if !['youtube', 'web'].includes(item.type)}
      <button
        class=" flex w-full items-center justify-between rounded-xl px-2 py-1 text-left {idx ===
        selectedIdx
      ...
      >
        <div class="  flex items-center gap-1 text-black dark:text-gray-100">
          <Tooltip
            content={item?.legacy
              ? $i18n.t('Legacy')
              : item?.type === 'file'
  ...
  {#if isYoutubeUrl(query)}
    <button
      class="selected-command-option-button w-full rounded-xl bg-gray-50 px-2 py-1 text-left dark:bg-gray-800 dark:text-gray-100"
      type="button"
    ...
    >
      <div class="  line-clamp-1 flex items-center gap-1 text-black dark:text-gray-100">
