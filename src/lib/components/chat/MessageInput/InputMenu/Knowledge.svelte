    ...
    {:else}
      {#each items as item, idx (item.id)}
        ...
        >
          ...
          >
            <div class="flex w-full items-center gap-1 text-left text-black dark:text-gray-100">
          ...
          <Tooltip content={$i18n.t('Show Files')} placement="top">
            <button
              type="button"
              class=" ml-2 opacity-50 transition hover:opacity-100"
              on:click={() => {
        ...
        {#if selectedItem && selectedItem.id === item.id}
          <div class="mb-1 flex flex-col gap-0.5 pl-3">
            ...
            {:else if selectedFileItemsTotal === 0}
              <div class=" px-2 py-0.5 text-xs text-gray-500 italic dark:text-gray-400">
                {$i18n.t('No files in this knowledge base.')}
              </div>
            {:else}
              {#each selectedFileItems as file, fileIdx (file.id)}
                <button
                  class=" flex w-full items-center justify-between rounded-xl px-2.5 py-1 text-left text-sm hover:bg-gray-50 hover:dark:bg-gray-800 hover:dark:text-gray-100"
                  type="button"
                  on:click={() => {
                    console.log(file);
                    onSelect({
