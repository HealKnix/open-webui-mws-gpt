...
{#if $showCallOverlay}
  <div class="flex h-full max-h-[100dvh] w-full max-w-lg flex-col justify-between p-3 md:p-6">
    {#if camera}
      <button
        type="button"
        class="flex h-20 min-h-20 w-full items-center justify-center"
        on:click={() => {
          ...
          <div
            class=" {rmsLevel * 100 > 4
              ...
              : rmsLevel * 100 > 2
                ...
                : rmsLevel * 100 > 1
                  ? 'size-14'
                  : 'size-12'}  rounded-full bg-cover bg-center bg-no-repeat transition-all"
    ...
    <div class="flex h-full max-h-full w-full flex-1 items-center justify-center">
      {#if !camera}
        <button
          type="button"
          on:click={() => {
            ...
            <div
              class=" {rmsLevel * 100 > 4
                ...
                : rmsLevel * 100 > 2
                  ...
                  : rmsLevel * 100 > 1
                    ? 'size-44'
                    : 'size-40'} rounded-full bg-cover bg-center bg-no-repeat transition-all"
              style={`background-image: url('${WEBUI_API_BASE_URL}/models/model/profile/image?id=${model?.id}&lang=${$i18n.language}&voice=true');`}
            />
          {/if}
        </button>
      {:else}
        <div class="video-container relative flex h-full max-h-full w-full px-2 pt-2 pb-4 md:py-6">
          <!-- svelte-ignore a11y-media-has-caption -->
          <video
            id="camera-feed"
            autoplay
            class="h-full min-w-full rounded-2xl object-cover object-center"
            playsinline
          />

          <canvas id="camera-canvas" style="display:none;" />

          <div class=" absolute top-4 left-4 md:top-8">
            <button
              type="button"
              class="cursor-pointer rounded-full bg-black/10 p-1.5 text-white backdrop-blur-xl"
              on:click={() => {
                stopCamera();
              }}
            >
    ...
    <div class="flex w-full items-center justify-between pb-2">
      <div>
        {#if camera}
          ...
          >
            <button class=" rounded-full bg-gray-50 p-3 dark:bg-gray-900" type="button">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 20 20"
                fill="currentColor"
        ...
        {:else}
          <Tooltip content={$i18n.t('Camera')}>
            <button
              class=" rounded-full bg-gray-50 p-3 dark:bg-gray-900"
              type="button"
              on:click={async () => {
