...
<div class="flex items-center gap-2 px-2 py-1.5">
  ...
  <div class="flex min-w-0 flex-1 items-center gap-2">
    {#if files.length > 0}
      <div class="flex shrink-0 items-center gap-1">
        {#each files as file}
          ...
          {:else}
            <div
              class="flex items-center rounded-md bg-gray-100 px-1.5 py-0.5 text-xs text-gray-500 dark:bg-gray-800 dark:text-gray-400"
            >
    ...
    {#if content}
      <p class="truncate text-sm text-gray-600 dark:text-gray-300">{content}</p>
  ...
  <div class="flex shrink-0 items-center gap-1">
    ...
    <Tooltip content={$i18n.t('Send now')}>
      <button
        type="button"
        class="p-1 text-gray-400 transition-colors hover:text-gray-700 dark:text-gray-500 dark:hover:text-gray-300"
