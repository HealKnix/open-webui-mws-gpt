...
{#if loaded}
  <div class="font-primary w-full">
    <div class=" inset-x-0 mx-auto flex justify-center bg-transparent">
      <div
      ...
      >
        <div class="relative">
          {#if autoScroll === false && history?.currentId}
            <div
              class=" pointer-events-none absolute -top-12 right-0 left-0 z-30 flex justify-center"
            >
              <button
                class=" pointer-events-auto rounded-full border border-gray-100 bg-white p-1.5 dark:border-none dark:bg-white/20"
                on:click={() => {
                  autoScroll = true;
                  scrollToBottom();
                }}
    ...
    <div class="bg-transparent">
      <div
        class="{($settings?.widescreenMode ?? null)
          ? 'max-w-full'
          : 'max-w-6xl'} inset-x-0 mx-auto px-2.5"
      >
        <div class="">
          <input
          ...
          <form
            class="flex w-full flex-col gap-1.5 {recording ? 'hidden' : ''}"
          ...
          >
            ...
            {#if messageQueue.length > 0}
              <div
                class="mx-2 mb-1 max-h-[25vh] overflow-x-hidden overflow-y-auto rounded-2xl border border-gray-100 bg-white px-1.5 py-0.5 dark:border-gray-800/50 dark:bg-gray-900/60"
              >
            ...
            >
              {#if atSelectedModel !== undefined}
                <div class="z-10 flex w-full flex-col px-3 pt-3 text-left">
                  <div class="flex w-full items-center justify-between">
                    <div class="flex items-center gap-2 pl-[1px] text-sm dark:text-gray-500">
                      <img
                        alt="model profile"
                        class="size-3.5 max-w-[28px] rounded-full object-cover"
              ...
              {#if files.length > 0}
                <div
                  class="mx-2 mt-2.5 flex flex-wrap items-center gap-2 pb-1.5"
                  dir={$settings?.chatDirection ?? 'auto'}
                >
              ...
              <div class="px-2.5">
                <div
                  class="scrollbar-hidden h-fit max-h-96 w-full resize-none overflow-auto bg-transparent px-1 pb-1 outline-hidden ltr:text-left rtl:text-right dark:text-gray-100 {files.length ===
                  0
                    ? atSelectedModel !== undefined
                      ? 'pt-1.5'
                      : 'pt-2.5'
                  ...
                  {#if prompt.split('\n').length > 2}
                    <div class="fixed top-0 right-0 z-20">
                      <div class="mt-2.5 mr-3">
                        <button
                          type="button"
                          class="rounded-lg p-1 hover:bg-gray-100/50 dark:hover:bg-gray-800/50"
              ...
              <div class=" mx-0.5 mt-0.5 mb-2.5 flex max-w-full justify-between" dir="ltr">
                <div class="ml-1 flex max-w-[80%] flex-1 items-center self-end">
                  ...
                  <div class="ml-1 flex gap-1.5">
                    {#if (selectedToolIds ?? []).length > 0}
                      ...
                      >
                        <button
                          class="flex translate-y-[0.5px] items-center gap-1 self-center rounded-lg px-1 text-gray-600 transition hover:text-gray-700 dark:text-gray-300 dark:hover:text-gray-200"
                      ...
                      {#if filter}
                        <Tooltip content={filter?.name} placement="top">
                          <button
                            ...
                            type="button"
                            class="group flex max-w-full items-center gap-1.5 overflow-hidden rounded-full p-[7px] text-sm transition-colors duration-300 focus:outline-hidden {selectedFilterIds.includes(
                              filterId,
                            )
                              ...
                              : 'bg-transparent text-gray-600 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-800 '} capitalize"
                          >
                            {#if filter?.icon}
                    ...
                    {#if webSearchEnabled}
                      <Tooltip content={$i18n.t('Web Search')} placement="top">
                        <button
                          ...
                          type="button"
                          class="group flex max-w-full items-center gap-1.5 overflow-hidden rounded-full p-[7px] text-sm transition-colors duration-300 focus:outline-hidden {webSearchEnabled ||
                    ...
                    {#if imageGenerationEnabled}
                      <Tooltip content={$i18n.t('Image')} placement="top">
                        <button
                          ...
                          type="button"
                          class="group flex max-w-full items-center gap-1.5 overflow-hidden rounded-full p-[7px] text-sm transition-colors duration-300 focus:outline-hidden {imageGenerationEnabled
                            ? ' border border-sky-200/40 bg-sky-50 text-sky-500 hover:bg-sky-100 dark:border-sky-500/20 dark:bg-sky-400/10 dark:text-sky-300 dark:hover:bg-sky-700/10'
                            : 'bg-transparent text-gray-600 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-800 '}"
                        >
                    ...
                    {#if codeInterpreterEnabled}
                      <Tooltip content={$i18n.t('Code Interpreter')} placement="top">
                        <button
                          ...
                          type="button"
                          class=" group flex max-w-full items-center gap-1.5 overflow-hidden p-[7px] text-sm transition-colors duration-300 {codeInterpreterEnabled
                            ? ' border border-sky-200/40 bg-sky-50 text-sky-500 hover:bg-sky-100 dark:border-sky-500/20 dark:bg-sky-400/10 dark:text-sky-300 dark:hover:bg-sky-700/10'
                            : 'bg-transparent text-gray-600 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-800 '} {($settings?.highContrastMode ??
                          false)
                            ? 'm-1'
                    ...
                    {#each pendingOAuthTools as pendingTool (pendingTool.id)}
                      <Tooltip content={$i18n.t('Click to connect')} placement="top">
                        <button
                          ...
                          type="button"
                          class="group flex max-w-full items-center gap-1.5 overflow-hidden rounded-full border border-amber-200/40 bg-amber-50 px-2 py-[5px] text-xs
														text-amber-600 transition-colors duration-300 hover:bg-amber-100 focus:outline-hidden dark:border-amber-500/20 dark:bg-amber-400/10 dark:text-amber-400 dark:hover:bg-amber-600/10"
                        >
                ...
                <div class="mr-1 flex shrink-0 gap-[0.5px] space-x-1 self-end">
                  {#if (taskIds && taskIds.length > 0) || (history.currentId && history.messages[history.currentId]?.done != true) || generating}
                    <div class=" flex items-center">
                      <Tooltip content={$i18n.t('Stop')}>
                        <button
                          class="rounded-full bg-white p-1.5 text-gray-800 transition hover:bg-gray-100 dark:bg-gray-700 dark:text-white dark:hover:bg-gray-800"
                          on:click={() => {
                            stopResponse();
                          }}
                        >
                    ...
                    {#if prompt !== '' && !history?.currentId && !$selectedTerminalId && ($config?.features?.enable_notes ?? false) && ($_user?.role === 'admin' || ($_user?.permissions?.features?.notes ?? true))}
                      ...
                      <Tooltip content={$i18n.t('Create note')} className=" flex items-center">
                        <button
                          ...
                          class=" -mr-1 self-center rounded-full p-1.5 text-gray-500 transition hover:text-gray-700 dark:text-gray-500 dark:hover:text-gray-200"
                          type="button"
                      ...
                      {#if $_user?.role === 'admin' || ($_user?.permissions?.chat?.stt ?? true)}
                        ...
                        <Tooltip content={$i18n.t('Dictate')}>
                          <button
                            ...
                            class=" mr-0.5 self-center rounded-full p-1.5 text-gray-600 transition hover:text-gray-700 dark:text-gray-300 dark:hover:text-gray-200"
                            type="button"
                            on:click={async () => {
                              try {
            ...
            {#if $config?.license_metadata?.input_footer}
              <div class=" marked line-clamp-1 text-center text-xs text-gray-500">
