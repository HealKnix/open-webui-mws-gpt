  ...
  const getFeatures = () => {
    ...
    if (
      currentModels.filter(
        (model) =>
          $models.find((m) => m.id === model)?.info?.meta?.capabilities?.web_search ?? true,
      ).length === currentModels.length
    ) {
...
<div
  class="transition-width h-screen max-h-[100dvh] duration-200 ease-in-out {$showSidebar
    ? '  md:max-w-[calc(100%-var(--sidebar-width))]'
    : ' '} flex w-full max-w-full flex-col"
  id="chat-container"
>
  {#if !loading}
    <div in:fade={{ duration: 50 }} class="flex h-full w-full flex-col">
      {#if $selectedFolder && $selectedFolder?.meta?.background_image_url}
        ...
        <div
          class="absolute top-0 left-0 z-0 h-full w-full bg-linear-to-t from-white to-white/85 dark:from-gray-900 dark:to-gray-900/90"
        />
      ...
      <PaneGroup direction="horizontal" class="h-full w-full">
        <Pane defaultSize={50} minSize={30} class="relative flex h-full max-w-full flex-col">
          ...
          <div id="chat-pane" class="@container z-10 flex w-full flex-auto flex-col overflow-auto">
            {#if ($settings?.landingPageMode === 'chat' && !$selectedFolder) || createMessagesList(history, history.currentId).length > 0}
              <div
                class=" scrollbar-hidden z-10 flex h-0 w-full max-w-full flex-auto flex-col justify-between overflow-auto pb-2.5"
              ...
              <div class=" pb-2 {dragged ? 'z-0' : 'z-10'}">
                ...
                <div
                  class="absolute right-0 bottom-1 left-0 line-clamp-1 text-center text-xs text-gray-500"
                >
