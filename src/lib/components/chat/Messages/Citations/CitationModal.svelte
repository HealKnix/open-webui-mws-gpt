...
<Modal size="lg" bind:show>
  <div>
    <div class=" flex justify-between px-4.5 pt-3 pb-2 dark:text-gray-300">
      <div class=" flex items-center self-center text-lg font-medium">
        {#if citation?.source?.name}
          ...
          {#if document?.metadata?.file_id || document.source?.url?.includes('http')}
            ...
            >
              <a
                class="line-clamp-1 grow underline hover:text-gray-500 dark:hover:text-gray-100"
    ...
    <div class="flex w-full flex-col px-5 pb-5 md:flex-row md:space-x-4">
      <div
        class="scrollbar-thin flex max-h-[22rem] w-full flex-col gap-1 overflow-y-scroll dark:text-gray-200"
      >
        {#each mergedDocuments as document, documentIdx}
          <div class="flex w-full flex-col gap-2">
            {#if document.metadata?.parameters}
              <div>
                <div class="mb-1 text-sm font-medium dark:text-gray-300">
                  {$i18n.t('Parameters')}
                </div>
            ...
            <div>
              <div
                class=" mb-1 flex w-fit items-center gap-2 text-sm font-medium dark:text-gray-300"
              >
                ...
                {#if showRelevance && document.distance !== undefined}
                  <Tooltip
                    ...
                    tippyOptions={{ duration: [500, 0] }}
                  >
                    <div class="my-1 flex w-fit items-center gap-2 text-sm dark:text-gray-400">
                      {#if showPercentage}
                        {@const percentage = calculatePercentage(document.distance)}

                        {#if typeof percentage === 'number'}
                          <span
                            class={`rounded-sm px-1 font-medium ${getRelevanceColor(percentage)}`}
                          >
              ...
              {:else}
                ...
                {#if $settings?.renderMarkdownInPreviews ?? true}
                  <div class="prose dark:prose-invert max-w-full text-sm">
                    <Markdown
                      content={isTruncated
                  ...
                  {/if}
                {:else}
                  <pre class="text-sm whitespace-pre-line dark:text-gray-400">{rawContent}</pre>
                {/if}
              {/if}
            </div>
          </div>
