...
{#key message.id}
  ...
  >
    <div class={`mt-1 hidden shrink-0 @lg:flex ltr:mr-3 rtl:ml-3 `}>
      <ProfileImage
      ...
      <div>
        <div class="chat-{message.role} markdown-prose w-full min-w-full">
          <div>
            ...
            {#if edit === true}
              <div class="my-2 w-full rounded-3xl bg-gray-50 px-5 py-3 dark:bg-gray-800">
                <textarea
        ...
        {#if !edit}
          <div
            ...
            class="buttons mt-0.5 flex justify-start overflow-x-auto text-gray-600 dark:text-gray-500"
          >
            {#if message.done || siblings.length > 1}
              {#if siblings.length > 1}
                <div class="flex min-w-fit self-center" dir="ltr">
                  <button
              ...
              {#if message.done}
                ...
                <Tooltip content={$i18n.t('Copy')} placement="bottom">
                  <button
                    ...
                    class="{isLastMessage || ($settings?.highContrastMode ?? false)
                      ? 'visible'
                      : 'invisible group-hover:visible'} copy-response-button rounded-lg p-1.5 transition hover:bg-black/5 hover:text-black dark:hover:bg-white/5 dark:hover:text-white"
                    on:click={() => {
                ...
                {#if message.usage}
                  ...
                  >
                    <button
                      ...
                      class=" {isLastMessage || ($settings?.highContrastMode ?? false)
                        ? 'visible'
                        : 'invisible group-hover:visible'} rounded-lg p-1.5 whitespace-pre-wrap transition hover:bg-black/5 hover:text-black dark:hover:bg-white/5 dark:hover:text-white"
                      on:click={() => {
                        console.log(message);
                      }}
                      id="info-{message.id}"
                ...
                {#if !readOnly}
                  {#if !$temporaryChatEnabled && ($config?.features.enable_message_rating ?? true) && ($user?.role === 'admin' || ($user?.permissions?.chat?.rate_response ?? true))}
                    <Tooltip content={$i18n.t('Good Response')} placement="bottom">
                      <button
                        ...
                        class="{isLastMessage || ($settings?.highContrastMode ?? false)
                          ? 'visible'
                          : 'invisible group-hover:visible'} rounded-lg p-1.5 hover:bg-black/5 dark:hover:bg-white/5 {(
                          message?.annotation?.rating ?? ''
                        ).toString() === '1'
                          ? 'bg-gray-100 dark:bg-gray-800'
                          : ''} transition hover:text-black disabled:cursor-progress disabled:hover:bg-transparent dark:hover:text-white"
                  ...
                  {#if $user?.role === 'admin' || ($user?.permissions?.chat?.regenerate_response ?? true)}
                    {#if $settings?.regenerateMenu ?? true}
                      <button
                        type="button"
                        class="regenerate-response-button hidden"
                        on:click={() => {
                    ...
                    {:else}
                      <Tooltip content={$i18n.t('Regenerate')} placement="bottom">
                        <button
                          ...
                          class="{isLastMessage
                            ? 'visible'
                            : 'invisible group-hover:visible'} regenerate-response-button rounded-lg p-1.5 transition hover:bg-black/5 hover:text-black dark:hover:bg-white/5 dark:hover:text-white"
                          on:click={() => {
                            showRateComment = false;
                            regenerateResponse(message);
                      ...
                      >
                        {#if action?.icon}
                          <div class="size-4">
                            <img
                              src={action.icon}
                              class="h-4 w-4 {action.icon.includes('data:image/svg')
                                ? 'dark:invert-[80%]'
                                : ''}"
