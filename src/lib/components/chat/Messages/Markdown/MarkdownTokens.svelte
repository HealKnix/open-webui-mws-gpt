  ...
  {:else if token.type === 'table'}
    <div class="group relative mb-2 w-full">
      <div class="scrollbar-hidden relative max-w-full overflow-x-auto">
        ...
        >
          <thead
            class="border-none bg-white text-xs text-gray-700 uppercase dark:bg-gray-900 dark:text-gray-400"
          >
            <tr class="">
            ...
            {#each token.rows as row, rowIdx}
              <tr class="bg-white text-xs dark:bg-gray-900">
                {#each row ?? [] as cell, cellIdx}
                  <td
                    class="w-max px-3! py-2! text-gray-900 dark:text-white {token.rows.length -
                      1 ===
                    rowIdx
                      ? ''
                      : 'dark:border-gray-850! border-b border-gray-50!'}"
      ...
      <div class=" invisible absolute top-1 right-1.5 z-20 flex gap-0.5 group-hover:visible">
      ...
      <ol start={token.start || 1} dir="auto">
        {#each token.items as item, itemIdx}
          <li class="text-start">
            {#if item?.task}
              <input
                class=" flex-shrink-0 -translate-x-1 translate-y-[1px]"
                type="checkbox"
