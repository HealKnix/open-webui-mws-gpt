...
{#if message?.arena}
  <div class="-mb-0.5 pt-1.5 text-xs font-medium">
    {$i18n.t('This response was generated by "{{model}}"', {
...
<div
  class=" dark:border-gray-850/30 my-2.5 rounded-xl border border-gray-100/30 px-4 py-3"
...
>
  ...
  <div class="flex w-full justify-center">
    <div class=" scrollbar-none relative w-fit overflow-x-auto">
      <div class="mt-1.5 flex w-fit gap-1 pb-2">
        <!-- 1-10 scale -->
        {#each Array.from({ length: 10 }).map((_, i) => i + 1) as rating}
          <button
            aria-label={$i18n.t('Rate {{rating}} out of 10', { rating })}
            class="dark:border-gray-850/30 dark:hover:bg-gray-850 size-7 border border-gray-100/30 text-sm hover:bg-gray-50 {detailedRating ===
            rating
              ? 'bg-gray-100 dark:bg-gray-800'
              : ''} rounded-full transition disabled:cursor-not-allowed disabled:bg-white disabled:text-gray-500 dark:disabled:bg-gray-900"
            on:click={() => {
      ...
      <div class="sticky top-0 right-0 bottom-0 left-0 flex justify-between text-xs">
        <div>
          1 - {$i18n.t('Awful')}
        </div>
  ...
  <div>
    {#if reasons.length > 0}
      <div class="mt-1.5 text-sm font-medium">{$i18n.t('Why?')}</div>

      <div class="mt-1.5 flex flex-wrap gap-1.5 text-sm">
        {#each reasons as reason}
          <button
            class="dark:border-gray-850/30 dark:hover:bg-gray-850 border border-gray-100/30 px-3 py-0.5 hover:bg-gray-50 {selectedReason ===
            reason
              ? 'bg-gray-100 dark:bg-gray-800'
  ...
  <div class="mt-2">
    <textarea
      bind:value={comment}
      class="w-full resize-none rounded-xl bg-transparent px-1 py-2 text-sm outline-hidden"
  ...
  {#if $config?.features.enable_community_sharing && message?.model}
    <div class="dark:border-gray-850/30 mt-3 border-t border-gray-100/30 pt-3">
      <a
        href={`https://openwebui.com/models?q=${encodeURIComponent(message.model)}`}
        target="_blank"
        class="dark:hover:bg-gray-850 flex w-full cursor-pointer items-center justify-between rounded-xl px-3 py-2 transition hover:bg-gray-50"
      >
