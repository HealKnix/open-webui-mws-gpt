...
{#if parentMessage}
  <div>
    <div
      class="scrollbar-hidden flex snap-x snap-mandatory overflow-x-auto"
      id="responses-container-{chatId}-{parentMessage.id}"
    >
      {#if $settings?.displayMultiModelResponsesInTabs ?? false}
        <div class="w-full">
          <div class=" dark:border-gray-850 mb-4.5 flex w-full border-b border-gray-200">
            <div
              class="scrollbar-none flex w-fit gap-2 overflow-x-auto bg-transparent pt-1 text-center text-sm font-medium"
                ...
                {#if groupedMessageIdsIdx[modelIdx] !== undefined && (groupedMessageIds[modelIdx]?.messageIds ?? []).length > 0}
                  ...
                  <button
                    class="min-w-fit {selectedModelIdx == modelIdx
                      ...
                      : ' border-transparent opacity-35'} border-b-2 px-2.5 pb-1.5 transition"
                    on:click={async () => {
      ...
      {:else}
        {#each Object.keys(groupedMessageIds) as modelIdx}
          {#if groupedMessageIdsIdx[modelIdx] !== undefined && groupedMessageIds[modelIdx].messageIds.length > 0}
            ...
            <div
              class=" m-1 w-full max-w-full snap-center border {history.messages[messageId]
                ?.modelIdx == modelIdx
                ? `dark:bg-gray-850 border-2 border-gray-100 bg-gray-50 dark:border-gray-800 ${
                    $mobile ? 'min-w-full' : 'min-w-80'
                  }`
                : `dark:border-gray-850/30 border-dashed border-gray-100/30 ${
                    $mobile ? 'min-w-full' : 'min-w-80'
        ...
        <div class="flex justify-end">
          <div class="w-full">
            {#if history.messages[messageId]?.merged?.status}
              ...
              <div class="mt-2 w-full rounded-xl py-2 pr-2 pl-5">
                <Name>
                  {$i18n.t('Merged Response')}

                  {#if message.timestamp}
                    <span
                      class=" invisible -mt-0.5 ml-0.5 self-center text-xs font-medium text-gray-400 uppercase group-hover:visible"
                    >
                ...
                <div class="markdown-prose mt-1 w-full min-w-full">
                  {#if (message?.content ?? '') === ''}
                    <Skeleton />
                  {:else}
          ...
          {#if isLastMessage}
            <div class=" mt-1 shrink-0 text-gray-600 dark:text-gray-500">
              <Tooltip content={$i18n.t('Merge Responses')} placement="bottom">
                <button
                  ...
                  class="{true
                    ? 'visible'
                    : 'invisible group-hover:visible'} rounded-lg p-1 transition hover:bg-black/5 hover:text-black dark:hover:bg-white/5 dark:hover:text-white"
                  on:click={() => {
