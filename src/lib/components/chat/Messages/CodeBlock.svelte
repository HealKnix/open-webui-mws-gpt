...
<div>
  <div
    class="relative {className} dark:border-gray-850/30 my-0.5 flex flex-col rounded-2xl border border-gray-100/30"
    dir="ltr"
  >
    {#if ['mermaid', 'vega', 'vega-lite'].includes(lang)}
      ...
      {:else}
        <div class="p-3">
          {#if renderError}
            <div
              class="mb-2 flex gap-2.5 rounded-2xl border border-red-600/10 bg-red-600/10 px-4 py-3"
            >
              {renderError}
            </div>
          {/if}
    ...
    {:else}
      <div
        class="sticky {stickyButtonsClassName} right-0 left-0 z-10 flex w-full items-center justify-end gap-2 rounded-t-2xl bg-white px-3 py-1.5 text-xs text-black dark:bg-black dark:text-white"
      >
        ...
        <div class="flex shrink-0 items-center gap-0.5">
          <button
            class="flex items-center gap-1 rounded-md border-none bg-white bg-none px-1.5 py-0.5 transition dark:bg-black"
          ...
          {#if ($config?.features?.enable_code_execution ?? true) && (lang.toLowerCase() === 'python' || lang.toLowerCase() === 'py' || (lang === '' && checkPythonCode(code)))}
            {#if executing}
              <div
                class="run-code-button cursor-not-allowed border-none bg-white bg-none p-0.5 dark:bg-black"
              >
                {$i18n.t('Running')}
              </div>
            {:else if run}
              <button
                class="run-code-button flex items-center gap-1 rounded-md border-none bg-white bg-none px-1.5 py-0.5 transition dark:bg-black"
                on:click={async () => {
                  code = _code;
                  await tick();
                  executePython(code);
          ...
          {#if save}
            <button
              class="save-code-button rounded-md border-none bg-white bg-none px-1.5 py-0.5 transition dark:bg-black"
              on:click={saveCode}
            >
          ...
          <button
            class="copy-code-button rounded-md border-none bg-white bg-none px-1.5 py-0.5 transition dark:bg-black"
      ...
      <div
        class="language-{lang} -mt-8 rounded-t-2xl {editorClassName
          ? editorClassName
          : executing || stdout || stderr || result
            ? ''
            : 'rounded-b-2xl'} overflow-hidden"
      >
        <div class=" bg-white pt-6.5 dark:bg-black"></div>
        ...
        {#if !collapsed}
          ...
          {:else}
            <pre
              class=" hljs overflow-x-auto p-4 px-5"
              style="border-top-left-radius: 0px; border-top-right-radius: 0px; {(executing ||
                stdout ||
                stderr ||
                result) &&
                'border-bottom-left-radius: 0px; border-bottom-right-radius: 0px;'}"><code
                class="language-{lang} rounded-t-none text-sm whitespace-pre"
        ...
        {:else}
          <div
            class="flex flex-col gap-2 rounded-b-2xl! bg-white px-4 pt-1 pb-2 text-xs dark:bg-black dark:text-white"
          >
      ...
      {#if !collapsed}
        <div
          id="plt-canvas-{id}"
          class="scrollbar-hidden max-w-full overflow-x-auto bg-gray-50 dark:bg-black dark:text-white"
        />

        {#if executing || stdout || stderr || result || files}
          <div
            class="flex flex-col gap-2 rounded-b-2xl! bg-gray-50 px-4 py-4 dark:bg-black dark:text-white"
          >
            {#if executing}
              <div class=" ">
                <div class=" mb-1 text-sm text-gray-500">{$i18n.t('STDOUT/STDERR')}</div>
            ...
            {:else}
              {#if stdout || stderr}
                <div class=" ">
                  <div class=" mb-1 text-sm text-gray-500">{$i18n.t('STDOUT/STDERR')}</div>
                  <div
                    class="font-mono text-sm whitespace-pre-wrap {stdout?.split('\n')?.length > 100
                      ? `max-h-96`
                      : ''}  overflow-y-auto"
                  >
                    {stdout || stderr}
              ...
              {#if result || files}
                <div class=" ">
                  <div class=" mb-1 text-sm text-gray-500">{$i18n.t('RESULT')}</div>
                  {#if result}
