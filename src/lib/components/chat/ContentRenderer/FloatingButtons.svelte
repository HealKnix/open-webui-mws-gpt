...
<div
  ...
  class="absolute z-9999 mt-1 rounded-lg text-xs"
  style="display: none"
>
  {#if responseContent === null}
    {#if !floatingInput}
      <div
        class="dark:bg-gray-850 text-medium flex shrink-0 flex-row rounded-xl border border-gray-100 bg-white p-0.5 shadow-xl dark:border-gray-800 dark:text-gray-100"
      >
        {#each actions as action}
          <button
            aria-label={action.label}
            class="flex min-w-fit items-center gap-1 rounded-xl px-1.5 py-[1px] transition hover:bg-gray-50 dark:hover:bg-gray-800"
            on:click={async () => {
    ...
    {:else}
      <div
        class="dark:bg-gray-850 flex w-72 rounded-full border border-gray-100 bg-white py-1 shadow-xl dark:border-gray-800 dark:text-gray-100"
      >
        <input
          type="text"
          id="floating-message-input"
          class="ml-5 w-full flex-1 bg-transparent text-sm outline-hidden"
        ...
        <div class="mr-1 ml-1">
          <button
            ...
            class="{floatingInputValue !== ''
              ...
              : 'disabled bg-gray-200 text-white dark:bg-gray-700 dark:text-gray-900'} m-0.5 self-center rounded-full p-1.5 transition"
            on:click={() => {
              actionHandler(selectedAction?.id);
            }}
          >
  ...
  {:else}
    <div
      class="dark:bg-gray-850 w-80 max-w-full rounded-3xl border border-gray-100 bg-white shadow-xl dark:border-gray-800 dark:text-gray-100"
    >
      <div
        class="dark:bg-gray-850 text-medium w-full rounded-3xl bg-white px-3.5 pt-3 dark:text-gray-100"
      >
      ...
      <div class="dark:bg-gray-850 text-medium w-full rounded-4xl bg-white dark:text-gray-100">
        <div
          class=" markdown-prose-xs max-h-80 w-full overflow-y-auto px-3.5 py-3"
