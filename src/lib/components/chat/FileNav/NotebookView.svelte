...
<div class="notebook-view">
  ...
  {#if baseUrl && apiKey && filePath}
      ...
      <div class="flex items-center select-none">
        ...
        {:else if kernelReady}
          <Tooltip content="Jupyter" placement="bottom"
            ><span class="inline-block size-1.5 rounded-full bg-green-500"></span></Tooltip
          >
        {:else}
          <Tooltip content={$i18n.t('No kernel')} placement="bottom"
            ><span class="inline-block size-1.5 rounded-full bg-gray-400 dark:bg-gray-600"
            ></span></Tooltip
          >
        {/if}
      </div>
    ...
    {#if kernelError}
      <div
        class="bg-red-50 px-2 py-1 font-mono text-[0.65rem] whitespace-pre-wrap text-red-600 dark:bg-red-950/30 dark:text-red-400"
      >
        {kernelError}
      </div>
    {/if}
    ...
    <div class="nb-cell nb-cell-{cell.cell_type}">
      {#if cell.cell_type === 'markdown'}
        ...
        {:else}
          ...
          <div
            class="nb-markdown prose dark:prose-invert max-w-full cursor-text text-sm"
            role="textbox"
            tabindex="0"
