...
<div class="px-2 py-1">
  <button
    class="flex w-full items-center gap-1 text-xs font-medium text-gray-500 transition hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"
  ...
  >
    ...
    <span class="ml-auto flex items-center gap-1">
      ...
      <Tooltip content={$i18n.t('Refresh')}>
        <button
          class="rounded p-0.5 text-gray-400 transition hover:bg-gray-100 hover:text-gray-600 dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-400"
  ...
  {#if expanded}
    <div class="mt-1 max-h-[150px] space-y-0.5 overflow-y-auto">
      {#if ports.length === 0}
        <div class="px-1 py-1 text-xs text-gray-400 dark:text-gray-500">
          {$i18n.t('No servers detected')}
        </div>
      {:else}
        {#each ports as port}
          <button
            class="group flex w-full items-center gap-2 rounded px-1.5 py-1 text-xs transition hover:bg-gray-100 dark:hover:bg-gray-800"
            on:click={() => previewPort(port.port)}
          >
            <span class="shrink-0 font-mono text-blue-500 dark:text-blue-400">
              :{port.port}
            </span>
            <span class="flex-1 truncate text-left text-gray-500 dark:text-gray-400">
              {port.process ?? ''}
            </span>
            <Tooltip content={$i18n.t('Open in new tab')}>
              ...
              <span
                role="button"
                tabindex="-1"
                class="shrink-0 rounded p-0.5 text-gray-400 opacity-0 transition group-hover:opacity-100 hover:bg-gray-200 dark:text-gray-500 dark:hover:bg-gray-700"
