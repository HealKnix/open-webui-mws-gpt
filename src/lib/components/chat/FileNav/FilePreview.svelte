...
<div
  class="flex-1 {fileImageUrl !== null || (fileOfficeSlides !== null && fileOfficeSlides.length > 0)
    ? 'overflow-hidden'
    : 'overflow-y-auto'} relative h-full min-h-0 min-w-0"
>
  <!-- File preview -->
  {#if fileLoading}
    <div class="flex h-full items-center justify-center"><Spinner className="size-4" /></div>
  {:else if fileImageUrl !== null}
    <div class="flex h-full w-full items-center justify-center" use:initImagePanzoom>
      <img
        src={fileImageUrl}
  ...
  {:else if fileVideoUrl !== null}
    <div class="flex h-full w-full items-center justify-center bg-black">
      ...
      <video src={fileVideoUrl} controls class="max-h-full max-w-full">
  ...
  {:else if fileOfficeHtml !== null}
    <div class="flex h-full flex-col">
      ...
      {#if excelSheetNames.length > 1}
        <div
          class="flex items-center gap-1 overflow-x-auto border-t border-gray-100 px-3 py-1.5 dark:border-gray-800"
        >
          {#each excelSheetNames as sheet}
            <button
              class="shrink-0 rounded-md px-3 py-1 text-xs transition-colors
  ...
  {:else if fileOfficeSlides !== null && fileOfficeSlides.length > 0}
    <div class="flex h-full flex-col">
      ...
      {#if fileOfficeSlides.length > 1}
        <div
          class="flex items-center justify-center gap-3 border-t border-gray-100 px-3 py-2 text-xs text-gray-500 dark:border-gray-800"
        >
          <button
  ...
  {:else if fileContent !== null}
    ...
    {:else if isMarkdown && !showRaw}
      <div bind:this={markdownEl} class="prose dark:prose-invert max-w-full p-3 text-sm">
        {@html renderedHtml}
      </div>
    {:else if isMarkdown && showRaw}
      <div class="absolute inset-0">
    ...
    {:else if isCsv && !showRaw && csvRows.length > 0}
      <div class="absolute inset-0 overflow-auto px-3 pb-3">
        <table class="csv-table border-collapse font-mono text-xs">
          <thead>
            <tr>
    ...
    {:else if isJson && !showRaw && jsonError}
      <div class="p-3 text-xs">
        <div class="mb-2 text-red-500">JSON parse error: {jsonError}</div>
        <pre
          class="font-mono text-xs leading-relaxed break-all whitespace-pre-wrap text-gray-800 dark:text-gray-200">{fileContent}</pre>
      </div>
    {:else if isSvg && !showRaw && fileContent}
      <div class="svg-preview flex h-full w-full items-center justify-center overflow-auto p-3">
    ...
    {:else if isSvg && highlightedHtml && !showRaw}
      <div class="shiki-preview h-full overflow-auto text-xs">
    ...
    {:else if editing}
      <textarea
        ...
        class="h-full w-full resize-none border-none bg-transparent p-3 font-mono text-xs leading-relaxed break-all whitespace-pre text-gray-800 outline-none dark:text-gray-200"
        spellcheck="false"
      />
    {:else}
      <pre
        class="p-3 font-mono text-xs leading-relaxed break-all whitespace-pre-wrap text-gray-800 dark:text-gray-200">{fileContent}</pre>
    {/if}
  {:else}
