...
<li class="group">
  <div
    class="flex w-full items-center transition
			...
			{dragOverFolder
      ? 'bg-blue-50 ring-1 ring-blue-400 ring-inset dark:bg-blue-900/30 dark:ring-blue-500'
      : ''}"
    ...
    >
      ...
      {#if renaming}
        ...
        <input
          ...
          class="min-w-0 flex-1 rounded border border-gray-200 bg-transparent px-1.5 py-0.5 text-xs text-gray-800 outline-none focus:border-blue-400 dark:border-gray-700 dark:text-gray-200 dark:focus:border-blue-500"
          on:keydown={(e) => {
            if (e.key === 'Enter') {
              e.preventDefault();
              submitRename();
      ...
      {#if entry.type === 'file' && entry.size !== undefined && !renaming}
        <span class="shrink-0 text-xs text-gray-400">{formatFileSize(entry.size)}</span>
      {/if}
    </button>

    <Dropdown align="end" sideOffset={4}>
      <button
        class="mr-1 shrink-0 rounded-lg p-0.5 text-gray-400
					transition hover:bg-gray-100 hover:text-gray-600 dark:text-gray-500
      ...
      <div slot="content">
        <div
          class="dark:bg-gray-850 z-[9999999] min-w-[150px] rounded-2xl border border-gray-100 bg-white p-1 shadow-lg dark:border-gray-800 dark:text-white"
        >
          <button
            type="button"
            class="flex w-full items-center gap-2 rounded-xl px-3 py-1.5 text-sm transition select-none hover:bg-gray-50 dark:hover:bg-gray-800"
            on:click={(e) => {
              e.stopPropagation();
              const path =
