...
<Modal size="sm" bind:show>
  <div>
    <div class=" flex justify-between px-5 pt-4 pb-2 dark:text-gray-300">
      <div class=" self-center text-lg font-medium">{$i18n.t('Valves')}</div>
      <button
        class="self-center"
        on:click={() => {
          show = false;
      ...
      <div class=" flex w-full flex-col sm:flex-row sm:justify-center sm:space-x-6">
        ...
        >
          ...
          <div class="flex justify-end pt-3 text-sm font-medium">
            <button
              class="flex items-center gap-2 rounded-full bg-black px-3.5 py-1.5 text-sm font-medium whitespace-nowrap text-white transition hover:bg-gray-900 dark:bg-white dark:text-black dark:hover:bg-gray-100 {saving
                ? ' cursor-not-allowed'
                : ''}"
              type="submit"
              disabled={saving}
