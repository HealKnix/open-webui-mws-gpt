<script lang="ts">
  ...
  import { cn, splitStream } from '$lib/utils';
...
- card: props: { title, subtitle, description, image, badge, price, rating }.
- card-grid: responsive grid (1/2/3 columns). Children are cards.
- button: props: { label (display text), action_text (text sent to chat on click), color ("primary"|"secondary"|"foreground"), variant ("solid"|"flat"|"outline"|"ghost") }.
- text: props: { value }.
- badge: props: { value }.
...
<div
  class={cn(
    'flex h-full w-full flex-col border-l border-gray-100 bg-white dark:border-gray-800 dark:bg-gray-950',
    className,
  )}
>
  <!-- Header -->
