      ...
      <div class="flex w-full justify-end gap-1.5">
        ...
        {#if $user?.role === 'admin' || $user?.permissions?.workspace?.skills}
          ...
          >
            ...
            <div class=" hidden text-xs md:ml-1 md:block">{$i18n.t('New Skill')}</div>
          </a>
        {/if}
      </div>
    </div>
      ...
      <div class=" my-2 grid gap-2 px-3 lg:grid-cols-2">
        {#each filteredItems as skill}
          <Tooltip content={skill?.description ?? skill?.id}>
            <div
              class=" flex w-full space-x-4 rounded-2xl px-3 py-2.5 text-left transition {skill.write_access
    ...
    {:else}
      <div class=" my-16 mb-24 flex h-full w-full flex-col items-center justify-center">
        <div class="max-w-md text-center">
          ...
          <div class=" mb-1 text-lg font-medium">{$i18n.t('No skills found')}</div>
