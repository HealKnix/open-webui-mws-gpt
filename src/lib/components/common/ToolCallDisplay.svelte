...
<div {id} class={className}>
  {#if !grouped && embeds && Array.isArray(embeds) && embeds.length > 0}
    ...
    <div class="w-full cursor-pointer py-1">
  ...
  {:else}
    ...
    >
      <div
        class="flex w-full max-w-full items-center gap-1.5 font-medium {isExecuting
          ? 'shimmer'
          : ''}"
      >
        <!-- Status icon -->
        ...
        <!-- Chevron -->
        <div class="flex shrink-0 translate-y-[1px] self-center">
          {#if open}
    ...
    {#if open}
      <div transition:slide={{ duration: 300, easing: quintOut, axis: 'y' }}>
        <div class="dark:border-gray-850/30 my-1.5 space-y-3 rounded-2xl border border-gray-50 p-3">
          <!-- Input -->
          {#if args}
            <div>
              <div
                class="mb-1.5 px-1 text-[10px] font-medium tracking-wider text-gray-400 uppercase dark:text-gray-500"
              >
                {$i18n.t('Input')}
              </div>
                ...
                <div class="space-y-0.5 px-1">
                  {#each Object.entries(parsedArgs) as [key, value]}
                    <div class="flex gap-2 py-0.5 text-xs">
                      <span class="shrink-0 font-medium text-gray-600 dark:text-gray-400"
                        >{key}</span
                      >
            ...
            <div>
              ...
              <div class="w-full max-w-none!">
                ...
                {:else}
                  ...
                  <pre
                    class="font-mono text-xs break-words whitespace-pre-wrap text-gray-600 dark:text-gray-300">{isTruncated
