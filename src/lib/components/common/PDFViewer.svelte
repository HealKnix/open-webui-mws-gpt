...
<div class="relative {className}">
  ...
  <div class="h-full overflow-y-auto" bind:this={outerContainer}>
    <div bind:this={sceneElement} class="w-full"></div>
  </div>

  {#if !loading && !error && pdfDoc}
    <div
      class="dark:bg-gray-850/90 absolute bottom-3 left-1/2 z-10 flex -translate-x-1/2 items-center gap-0.5 rounded-lg border border-gray-200/60 bg-white/90 px-1 py-0.5 shadow-lg backdrop-blur-sm dark:border-gray-700/60"
    >
      <button
      ...
      <button
        class="min-w-[3rem] rounded-md px-1.5 py-1 text-center text-[11px] font-medium text-gray-500 tabular-nums transition hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800"
        on:click={resetView}
