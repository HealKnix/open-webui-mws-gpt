      ...
      <div>
        <div class="flex w-full flex-col items-center justify-between gap-1 md:flex-row">
          <div class=" flex flex-wrap gap-1 text-xs text-gray-500">
            {#if item?.type === 'collection'}
              {#if item?.type}
                <div class="shrink-0 capitalize">{item.type}</div>
                •
              {/if}
            ...
            {#if item.size}
              <div class="shrink-0 capitalize">{formatFileSize(item.size)}</div>
              •
            {/if}
    ...
    <div class="max-h-[75vh] overflow-auto">
      {#if !loading}
        ...
        {#if isAudio || isPDF || isExcel || isCode || isMarkdown || isDocx || isPptx}
          <div
            class="scrollbar-none dark:border-gray-850/30 mb-2.5 flex w-full overflow-x-auto border-b border-gray-50 bg-transparent text-center text-sm font-medium dark:text-gray-200"
          >
            <button
              class="min-w-fit border-b px-4 py-1.5 {selectedTab === ''
                ? ' '
                : ' border-transparent text-gray-300 hover:text-gray-700 dark:text-gray-600 dark:hover:text-white'} transition"
              type="button"
              on:click={() => {
                selectedTab = '';
            ...
            <button
              class="min-w-fit border-b px-4 py-1.5 {selectedTab === 'preview'
                ? ' '
                : ' border-transparent text-gray-300 hover:text-gray-700 dark:text-gray-600 dark:hover:text-white'} transition"
              type="button"
              on:click={() => {
        ...
        {#if isImage}
          <div class="relative max-h-[70vh] w-full overflow-hidden">
            <div class="absolute top-2 right-2 z-10">
              <Tooltip content={$i18n.t('Reset view')}>
                <button
                  class="dark:bg-gray-850/80 rounded-lg bg-white/80 p-1.5 text-gray-500 shadow-sm backdrop-blur-sm transition hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800"
        ...
        {:else if selectedTab === ''}
          {#if item?.file?.data}
            ...
            {#if $settings?.renderMarkdownInPreviews ?? true}
              <div
                class="scrollbar-hidden prose dark:prose-invert max-h-96 max-w-full overflow-scroll text-sm"
              >
                <Markdown
              ...
              {#if isTruncated}
                <button
                  class="mt-1 text-xs text-gray-500 transition hover:text-gray-700 dark:hover:text-gray-300"
                  on:click={() => {
              ...
              {/if}
            {:else}
              <div class="scrollbar-hidden max-h-96 overflow-scroll text-xs whitespace-pre-wrap">
                {rawContent}
              </div>
            {/if}
        ...
        {:else if selectedTab === 'preview'}
          {#if isAudio}
            <audio
              src={`${WEBUI_API_BASE_URL}/files/${item.id}/content`}
              class="mb-2 w-full rounded-lg border-0"
              controls
              playsinline
            />
          {:else if isPDF}
          ...
          {:else if isExcel}
            ...
            {:else}
              {#if excelSheetNames.length > 1}
                ...
                >
                  {#each excelSheetNames as sheetName}
                    <button
                      class="min-w-fit border-b px-4 py-1.5 {selectedSheet === sheetName
                        ? ' '
                        : ' border-transparent text-gray-300 hover:text-gray-700 dark:text-gray-600 dark:hover:text-white'} transition"
                      type="button"
                      on:click={() => {
              ...
              {#if excelHtml}
                <div class="office-preview max-h-[60vh] overflow-auto">
                  {@html excelHtml}
                </div>
              {:else}
                <div class="p-4 text-sm text-gray-500">No content available</div>
              {/if}
            {/if}
          {:else if isCode}
            <div class="scrollbar-hidden relative max-h-[60vh] overflow-scroll text-sm">
              <CodeBlock
          ...
          {:else if isMarkdown}
            <div
              class="scrollbar-hidden prose dark:prose-invert max-h-[60vh] max-w-full overflow-scroll text-sm"
            >
              <Markdown content={item.file.data.content} id="markdown-viewer" />
            </div>
          {:else if isDocx}
            {#if docxError}
              <div class="p-4 text-sm text-red-500">{docxError}</div>
            {:else if docxHtml}
              <div
                class="office-preview prose dark:prose-invert max-h-[60vh] max-w-full overflow-auto p-4 text-sm"
              >
                {@html docxHtml}
              </div>
            {:else}
          ...
          {:else if isPptx}
            {#if pptxError}
              <div class="p-4 text-sm text-red-500">{pptxError}</div>
              ...
              <div class="max-h-[60vh] overflow-auto">
                <div class="flex justify-center p-4">
                  <img
                    ...
                    class="max-h-[50vh] max-w-full rounded-md object-contain shadow-lg"
                    draggable="false"
                  />
                </div>
