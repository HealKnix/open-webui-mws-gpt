...
<div class="mb-1.5 flex items-center pt-1.5">
  <div class="mr-1 flex items-center">
    <button
      class="rounded-lg bg-transparent p-0.5 transition"
      on:click={() => {
        show = !show;
      }}
    >
...
<div class="@container flex flex-1 flex-col items-center">
  <div class=" flex h-full w-full flex-col justify-between overflow-y-auto">
    <div class="relative mx-auto h-full w-full md:px-0">
      <div class=" flex h-full flex-col">
        <div
          class=" scrollbar-hidden flex h-0 w-full flex-auto flex-col justify-between overflow-auto pb-2.5"
        ...
        <div class=" pb-[1rem]">
          {#if selectedContent}
            <div class="markdown-prose-xs w-full rounded-xl px-2.5 py-3 text-xs">
              <blockquote>
          ...
          <MessageInput
            ...
            {onStop}
          >
            <div slot="menu" class="flex w-full items-center justify-between gap-2 pr-1">
              <div>
                <Tooltip content={$i18n.t('Edit')} placement="top">
                  <button
                    ...
                    type="button"
                    class="flex max-w-full items-center gap-1.5 overflow-hidden rounded-full px-2 py-2 text-sm transition-colors duration-300 hover:bg-gray-50 focus:outline-hidden @xl:px-2.5 dark:hover:bg-gray-800 {editEnabled
                      ? ' bg-sky-50 text-sky-500 dark:bg-sky-200/5 dark:text-sky-300'
                      : 'bg-transparent text-gray-600 dark:text-gray-300 '} disabled:pointer-events-none disabled:opacity-50"
                  >
                    <PencilSquare className="size-4" strokeWidth="1.75" />
                    <span
                      class="block overflow-hidden pr-0.5 leading-none text-ellipsis whitespace-nowrap"
                      >{$i18n.t('Edit')}</span
                    >
                  </button>
                </Tooltip>
              ...
              <Tooltip content={selectedModelId}>
                <select
                  class=" -mx-0.5 w-full rounded-lg bg-transparent px-2 py-1 pr-5 text-right text-sm outline-hidden"
