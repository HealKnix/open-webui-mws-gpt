...
<div class="group flex flex-col gap-1">
  <div class="flex items-center justify-between pt-1">
    <div class="min-w-[6rem] rounded-lg py-1 text-left text-sm font-semibold uppercase transition">
      {$i18n.t(message.role)}
    </div>
    ...
    <div class="flex items-center gap-2">
      <Tooltip placement="top" content={$i18n.t('Insert')}>
        <button
          class=" text-transparent transition group-hover:text-gray-500 dark:hover:text-gray-300"
          on:click={() => {
            onInsert();
          }}
        >
