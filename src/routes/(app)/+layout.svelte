...
{#if version && compareVersion(version.latest, version.current) && ($settings?.showUpdateToast ?? true)}
  <div class=" absolute right-8 bottom-8 z-50" in:fade={{ duration: 100 }}>
    <UpdateInfoToast
      {version}
      on:close={() => {
...
{#if $user}
  <div class="app relative">
    <div
      class=" flex h-screen max-h-[100dvh] flex-row justify-end overflow-auto bg-white text-gray-700 dark:bg-gray-900 dark:text-gray-100"
    >
      {#if !['user', 'admin'].includes($user?.role)}
        <AccountPending />
      {:else}
        {#if localDBChats.length > 0}
          <div class="fixed z-50 flex h-full w-full">
            <div
              class="absolute flex h-full w-full justify-center bg-white/20 backdrop-blur-md dark:bg-gray-900/50"
            >
              <div class="m-auto flex flex-col justify-center pb-44">
                <div class="max-w-md">
                  <div class="z-50 text-center text-2xl font-medium dark:text-white">
                    {$i18n.t('Important Update')}<br />
                    {$i18n.t('Action Required for Chat Log Storage')}
                  </div>

                  <div class=" mt-4 w-full text-center text-sm dark:text-gray-200">
                    {$i18n.t(
                  ...
                  <div class=" group relative mx-auto mt-6 w-fit">
                    <button
                      class="relative z-20 flex rounded-full border border-gray-100 bg-white px-5 py-2 text-sm font-medium transition hover:bg-gray-100 dark:border-none"
                      on:click={async () => {
        ...
        {:else}
          <div
            class="flex h-full w-full flex-1 items-center justify-center {$showSidebar
